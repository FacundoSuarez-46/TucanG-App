...
function saveUser(req, res){
	...
	if(params.name && params.surname && 
	   params.nick && params.desc && params.dep && params.email && params.password){
...
function getFilterUsers(req, res){
	...
	User.find({desc: word}).collation({ locale: 'en', strength: 2 }).sort('-created_at').populate('user').exec((err, users) => {

		if(err) return res.status(404).send({message: 'error en la peticion'});

		if(!users) return res.status(500).send({message: 'No hay usuarios con esa categoria'});
		
		if(users){
			console.log(req.body.word); console.log(user); return res.status(200).send({users}); 
		}    
...
function getAllUsers(req, res){
	...
	find.populate('users').exec((err, users) => {
		if(err) return res.status(500).send({message: 'Error en la petición'});
