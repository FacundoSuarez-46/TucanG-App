...
function savePublication(req, res){
	...
	if(!params.text, !params.title) return res.status(200).send({message: 'Debes enviar un texto!!'});
...
//obtener publicaciones por busqueda
function getFilterPublications(req, res){

	var word = req.body.word;

	Publication.find({type: word}).collation({ locale: 'en', strength: 2 }).sort('-created_at').populate('user').exec((err, publications, total) => {

		if(err) return res.status(404).send({message: 'error en la peticion'});

		if(!publications) return res.status(500).send({message: 'No hay publicaciones con esa categoria'}); console.log(req.body);
		
		if(publications){
			console.log(req.body.word); return res.status(200).send({publications}); 
		}    
